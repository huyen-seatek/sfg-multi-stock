...
class Wcmlim_Location_Popup
{
    ...
    public function wcmlim_set_inline_css(){
        ...
        if(isset($popuppos) && $popuppos !== "default"){
            if($popuppos == "bottom"){  echo "<style> .set-def-store-popup-btn{margin-right: -39px;margin-bottom: -40px;}</style>"; }
            if($popuppos == "top"){  echo "<style> .set-def-store-popup-btn{ padding-top: 35px !important;margin-right: -43px;}</style>"; }
            if($popuppos == "left"){  echo "<style> .set-def-store-popup-btn{ padding-right: 5px !important; margin-left:10px;padding-bottom: 2px !important }</style>"; }
            if($popuppos == "right"){  echo "<style> .set-def-store-popup-btn{ margin-right: -90px;margin-bottom: -15px;}</style>"; }
        }
    ...
    public function wcmlim_locations_popup_cb()
    {
        ...
        if (isset($_COOKIE['wcmlim_selected_location']) && $_COOKIE['wcmlim_selected_location'] != -1) {
            ...
            $select_location = isset($locations_list[$_COOKIE['wcmlim_selected_location']]) ? $locations_list[$_COOKIE['wcmlim_selected_location']] : null;
